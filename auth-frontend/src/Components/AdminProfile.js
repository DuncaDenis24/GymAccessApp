...
const AdminProfile = ({ onLogout }) => {
    ...
    const hasChanges = () => {
        return (
            ...
            formData.surname !== admin?.surname ||
            formData.email !== admin?.email ||
            formData.phone !== admin?.phone ||
            formData.profilePicture !== admin?.photo
        );
    };
    ...
    const handleConfirmCancelEdit = () => {
        setFormData({
            ...
            email: admin.email,
            phone: admin.phone,
            profilePicture: admin.photo || profilePicture
        });
        setIsEditing(false);
        setShowCancelEditing(false);
    };

    if (!admin) return <div className="loading-overlay">
